...
def _get_git_commit() -> str | None:
    ...
    try:
        ...
        logger.debug(
            "Attempting to read git commit from .git directory", git_dir=str(git_dir)
        )
        if git_dir.exists():
            ...
            if head.startswith("ref:"):
                ...
                logger.debug(
                    "Successfully read commit from .git/HEAD ref",
                    commit=commit,
                    ref=ref,
                )
                return commit
            logger.debug(
                "Successfully read commit from .git/HEAD (detached)", commit=head
            )
            return head
    ...
    try:
        ...
        logger.debug(
            "Successfully read commit from _build_metadata.py", commit=GIT_COMMIT
        )
        return GIT_COMMIT
    except Exception as e:
